...
def start(message):
    ...
    bot.send_message(message.chat.id, "🔸Я бот🤖, который может:\n"
                                      ...
                                      "Перед тем как начать работу нажмите Help❓ ", reply_markup=markup)
...
def one_click(message):
    if message.text == 'Меню 🗃':
        ...
        try:
            ...
            file = open(file_s, 'rb')
            bot.send_message(message.chat.id, f"Суммарная площадь капель: {get_sum()}\n"
                                              f"Площадь каждой капли будет записана в табличку😉")
        except Exception as e:
            bot.send_message(message.chat.id, "Извините, вы наверно не правильно поняли Help❓\n"
        ...
        try:
            ...
            bot.send_message(message.chat.id, "Извините, но вы не можете продолжить работу.😔\n"
        ...
        # bot.send_message(message.chat.id, 'Соня, поцелуй саню')
        # bot.register_next_step_handler(message, one_click)
    elif message.text == 'Help❓':
        markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
        item = types.KeyboardButton('⬅️ Назад')
        markup.add(item)
        bot.send_message(message.chat.id, "📌Для успешной работы бота отправьте фотографию капель в виде файла\n"
                                          "Далее нажмите <<Меню 🗃>>\n"
                                          ...
                                          "📌Если вы хотите получить данные о площадях в виде таблицы, нажмите"
                                          " <<Сохранить в xlsx>>", reply_markup=markup)
        bot.send_message(message.chat.id, "⁉Важно⁉\n"
                                          "Соблюдайте последовательность:\n"
                                          "<<Отрисовать контура капель>> ➡ <<Рассчитать площадь капель>> ➡ "
                                          "<<Сохранить в xlsx>>", reply_markup=markup)
    else:
        bot.send_message(message.chat.id, "Извините,мы не можем с вами общаться на отдалённые темы, "
                                          "прочтите пожалуйста Help❓ и узнайте как со мной работать😉")
